...
## Unit tests for Brewery.calculate_bulk_sell_payout()/calculate_ship_payout()
...
func test_bulk_sell_payout_at_full_quality_is_half_raw_cost() -> void:
	...
	var payout := Brewery.calculate_bulk_sell_payout(2.0, 1.0, 10)
...
func test_bulk_sell_payout_scales_down_with_quality() -> void:
	var poor := Brewery.calculate_bulk_sell_payout(2.0, 0.2, 10)
...
func test_bulk_sell_payout_clamps_quality_at_one() -> void:
	...
	var excellent := Brewery.calculate_bulk_sell_payout(2.0, 2.5, 10)
	var good := Brewery.calculate_bulk_sell_payout(2.0, 1.0, 10)
...
func test_bulk_sell_payout_zero_bottles_is_zero() -> void:
	assert_eq(Brewery.calculate_bulk_sell_payout(2.0, 1.0, 0), 0.0)


func test_ship_payout_applies_bar_price_multiplier() -> void:
	var payout := Brewery.calculate_ship_payout(2.0, 1.0, 10, 0.8)
...
func test_ship_payout_clamps_quality_floor() -> void:
	...
	var spoiled := Brewery.calculate_ship_payout(2.0, 0.05, 10, 0.8)
	var at_floor := Brewery.calculate_ship_payout(2.0, Brewery.SHIP_TO_BAR_QUALITY_CLAMP_MIN, 10, 0.8)
...
func test_ship_payout_clamps_quality_ceiling() -> void:
	...
	var masterful := Brewery.calculate_ship_payout(2.0, 2.5, 10, 0.8)
	var at_ceiling := Brewery.calculate_ship_payout(2.0, Brewery.SHIP_TO_BAR_QUALITY_CLAMP_MAX, 10, 0.8)
...
func test_ship_payout_beats_bulk_sell_at_the_same_quality() -> void:
	...
	var bulk := Brewery.calculate_bulk_sell_payout(2.0, 1.0, 10)
	var shipped := Brewery.calculate_ship_payout(2.0, 1.0, 10, 0.8)
